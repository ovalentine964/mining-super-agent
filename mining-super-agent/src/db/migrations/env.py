...
Supports PostGIS geometry types and pgvector.
"""
...
def _get_database_url() -> str:
    ...
    return config.get_main_option(
        "sqlalchemy.url",
        "postgresql+psycopg2://mining:mining@localhost:5432/mining",
    )
...
def run_migrations_offline() -> None:
    ...
    context.configure(
        ...
        # Include PostGIS / pgvector schemas so Alembic doesn't try to drop them
...
async def run_async_migrations() -> None:
    ...
    async with connectable.connect() as connection:
        # Ensure PostGIS extension exists before any migration runs
        await connection.execute(
            __import__("sqlalchemy").text("CREATE EXTENSION IF NOT EXISTS postgis")
        )
        await connection.execute(
            __import__("sqlalchemy").text("CREATE EXTENSION IF NOT EXISTS postgis_topology")
        )
        await connection.execute(
